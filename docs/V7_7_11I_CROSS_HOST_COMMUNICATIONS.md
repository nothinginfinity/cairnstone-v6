...
Primary implementation homes: existing CairnStone Worker and existing standalone web/PWA Console; host-supported in-chat MCP Apps are an optional projection of identical authorized contracts.
...
First complete 10i connection-principal isolation and honor 10j Tool Belt ceilings; preserve PR #49's already-planned 10k/11g/11h sequencing and do not interrupt ongoing 10i acceptance. Then freeze 11i privacy/identity contract. Only after controlled private-message MVP tests may an interactive in-chat composer ship. Email and video receive separate proposals/authorization and resource-cost budgets after the MVP, not presumed to be installed because MCP is connected.
