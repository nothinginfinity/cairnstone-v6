// V7.7.8a/b — Progressive Grounded Chat LOD
//
...
const MODEL_ALLOWLIST = new Set([MODEL_DEFAULT]);
...
export const GROUNDED_RESPONSE_CREATE_TOOL_DEFINITION = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      model: { type: "string", enum: [MODEL_DEFAULT] },
...
export const GROUNDED_RESPONSE_GET_TOOL_DEFINITION = {
  ...
  description:
    "V7.7.8a: load one persisted grounded response by response_id. Reports authority freshness versus current Scope heads without mutating accepted state. Does not eagerly materialize deeper response_lod levels.",
  inputSchema: {
    type: "object",
...
export const GROUNDED_RESPONSE_EXPAND_TOOL_DEFINITION = {
  ...
  description:
    "V7.7.8b: lazily materialize a deeper response_lod (2–5) for the SAME response_id / evidence / claim skeleton. Fails closed with stale_response/authority_changed when accepted heads moved unless view_original=true. Never regenerates an unconstrained independent answer. Never mutates chain/path HEADs.",
  inputSchema: {
    type: "object",
...
function publicResponseView(record, {
  ...
  if (authority) {
    view.authority_freshness = {
      status: authority.freshness,
      stale: authority.stale === true,
      current_authority_digest: authority.current_authority_digest || null,
      stored_authority_digest: authority.stored_authority_digest || record.authority_digest,
      viewed_original_snapshot: authority.viewed_original_snapshot === true,
      actions: authority.stale
...
export async function createGroundedResponseFromBody(body, env) {
  try {
    ...
    const model = typeof args.model === "string" && args.model ? args.model : MODEL_DEFAULT;
    if (!MODEL_ALLOWLIST.has(model)) {
      return { ok: false, error: "model_not_allowed", model, allowed_models: [...MODEL_ALLOWLIST], ...authorityClosedFields() };
    }
    ...
    const refreshNonce = typeof args.refresh_of === "string" && args.refresh_of.trim()
      ? `refresh:${args.refresh_of.trim()}:${new Date().toISOString()}`
      : null;
    ...
    const record = {
      ...
      parent_response_id: typeof args.refresh_of === "string" ? args.refresh_of : null,
      telemetry,
      created_at: now,
      updated_at: now
    };
  ...
  try {
    ...
    if (authority.stale && !viewOriginal) {
      return {
        ...
        actions: {
          view_original: {
            description: "Re-call expand with view_original=true to inspect the original snapshot.",
            params: { response_id: record.response_id, response_lod: targetLod, view_original: true }
          },
          refresh: {
            description: "Create a new grounded response against current authority (new response_id).",
            params: {
              question: record.question,
