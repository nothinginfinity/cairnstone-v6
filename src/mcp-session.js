...
export const LOAD_TOOLS_TOOL_DEFINITION = {
  ...
  description: "V7.6.2b/c experimental: request that this MCP session natively hydrate a bounded set of additional read+automatic CairnStone tools directly into tools/list, instead of reaching them generically via cairnstone_tool_search -> cairnstone_get_tool_contract -> cairnstone_tool_execute. Only tools whose existing broker classification is risk_class:'read' and authorization:'automatic', and whose exact definition exists in the live tool catalog, are eligible; unknown or ineligible tool_ids reject the entire request without persisting anything. On success, session-scoped hydration state is stored (never CairnStone accepted-state authority, never moves chain/path heads) and, when the requesting connection's Accept header lists text/event-stream, a notifications/tools/list_changed message is attempted in the same response stream before this tool's result. As of V7.6.2c, notification_delivered in the result is always false and portable_fallback_required is always true: live retest evidence (2026-09, mobile Claude client -- the primary target client for this console) showed the prior optimistic Accept-header-based signal was a false positive, since accepting that content-type on one request does not mean the client processes an inline notification frame or re-fetches tools/list. Callers on every client should therefore always use the portable tool_search -> get_tool_contract -> tool_execute path after calling this tool, treating any native call to a newly-listed tool as a bonus if it happens to work rather than something to rely on. The full /mcp surface and unhydrated /mcp/core behavior are both unaffected by this call.",
  inputSchema: {
    type: "object",
    required: ["tool_ids"],
    properties: {
